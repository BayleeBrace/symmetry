...
import { addDays, clock, isOpen, makeSlots, money, SERVICES, shopToday } from "../src/lib/booking-data.ts";
...
while(new Date(nextTuesday+"T12:00:00Z").getUTCDay()!==2)nextTuesday=addDays(nextTuesday,1);

test("any barber exposes exact barber-specific time, price and duration", () => {
  const slots = makeSlots(nextTuesday, "any", "fade", []);
  assert(slots.some((slot) => slot.barber === "dylan" && slot.time === 540 && slot.price === 17 && slot.duration === 50));
  assert(slots.some((slot) => slot.barber === "sean" && slot.price === 24 && slot.duration === 40));
});

test("busy periods remove every overlapping start", () => {
  const slots = makeSlots(nextTuesday, "sean", "cut", [{ barber: "sean", start: 570, duration: 30 }]);
