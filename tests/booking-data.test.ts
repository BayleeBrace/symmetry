...
import { addDays, clock, isOpen, makeSlots, money, SERVICES } from "../src/lib/booking-data.ts";
...
test("any barber exposes exact barber-specific time, price and duration", () => {
  const slots = makeSlots("2026-09-08", "any", "fade", []);
...
test("busy periods remove every overlapping start", () => {
  const slots = makeSlots("2026-09-08", "sean", "cut", [{ barber: "sean", start: 570, duration: 30 }]);
