...
export function cancellationDeadline(date: string, minutes: number, hours: number): Date {
  return new Date(shopInstant(date, minutes).getTime() - hours * 3600000);
}
export function deadlineLabel(date: string, minutes: number, hours: number): string {
