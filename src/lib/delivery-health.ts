...
export async function deliveryHealth() {
  ...
  if (failed.error || delayed.error)
    throw new Error("Message health could not load");
...
export async function alertDeliveryProblems() {
  ...
  if (!process.env.VAPID_PUBLIC_KEY || !process.env.VAPID_PRIVATE_KEY)
    return {
      alerted: false,
      reason: "Owner push is not configured",
      ...health,
    };
  const db = createAdminClient();
  ...
  if (!owners?.length)
    return { alerted: false, reason: "No active owner", ...health };
  ...
  if (!subs?.length)
    return {
      alerted: false,
      reason: "Owner must enable notifications on a device",
      ...health,
    };
  ...
  for (const sub of subs) {
    try {
      await webpush.sendNotification(
        sub.subscription,
        JSON.stringify({
          body: "Booking messages need attention. Open your diary to review.",
          url: "/staff",
        }),
        { timeout: 10000 },
      );
