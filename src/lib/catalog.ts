...
export async function getCatalog() {
  ...
  const [prices, barbers, services, hours] = await Promise.all([
    db.from("service_prices").select("*").eq("active", true),
    db.from("barbers").select("id,slug").eq("active", true),
    db
      .from("services")
      .select("id,slug,name")
      .eq("active", true)
  ...
  if (prices.error || barbers.error || services.error || hours.error)
      ...
      barbers.data
        .map((b) => {
          const p = prices.data.find(
            (p) => p.service_id === s.id && p.barber_id === b.id,
          );
          return [
            b.slug,
            p ? { price: p.price_pence / 100, duration: p.duration } : null,
          ];
        })
        .filter(([, p]) => p),
    ) as Record<BarberId, { price: number; duration: number }>,
  }));
  return {
    services: result,
    hours: Object.fromEntries(
      hours.data.map((h) => [
        h.iso_weekday % 7,
        h.open_minute === null ? null : [h.open_minute, h.close_minute],
      ]),
    ) as typeof OPENING_HOURS,
  };
}
export type Policy = {
...
export async function getPolicy(): Promise<Policy> {
  ...
  const { data, error } = await createAdminClient()
    ...
    .eq("id", true)
    .single();
  if (error) throw new Error("The cancellation policy could not load");
  return data;
}
