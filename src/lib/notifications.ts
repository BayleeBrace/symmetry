...
export async function processNotifications(){
 ...
 for(const job of jobs||[]){try{
  ...
  if(job.group_id){const {data:g,error}=await db.from('booking_groups').select('id,customers(name,email,phone)').eq('id',job.group_id).single();if(error||!g)throw new Error('Booking not found');const customer=g.customers as unknown as {name:string;email:string;phone:string};email=customer.email;phone=customer.phone;url=siteUrl()+'/bookings?token='+signLink('manage',g.id);
   ...
   if(job.kind==='reminder'&&b.status!=='booked'){await db.from('notification_jobs').update({status:'cancelled'}).eq('id',job.id);continue;}
   const service=b.services as unknown as {name:string};const barber=b.barbers as unknown as {name:string};
   subject=job.kind==='reminder'?'Your trim is tomorrow':job.kind==='cancelled'?'Your trim has been cancelled':job.kind==='moved'?'Your trim has moved':job.kind==='no_show'?'We missed you at Symmetry':job.kind==='running_late'?'Running-late update':'Your Symmetry booking';
