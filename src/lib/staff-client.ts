...
export async function renewStaffSession() {
  ...
  if (!refresh) {
    refresh = (async () => {
      const response = await fetch("/api/staff/session", { method: "PATCH" });
      const data = await response.json();
      if (!response.ok) {
        if (response.status === 401) throw new StaffSignInRequired(data.error);
        throw new Error(data.error || "Could not renew sign-in");
      }
      checkedAt = Date.now();
    })().finally(() => {
      refresh = null;
...
export async function staffApi(url: string, data?: unknown) {
  ...
  if (!response.ok)
    throw new Error(body.error || "Could not complete this action");
  return body;
}
