    ...
    <section className="staff-content">
      <div className="staff-toolbar">
        ...
        <div className="staff-toolbar-actions">
          <PushButton label="notifications" />
          <button
      ...
      <div
        className={`staff-tabs ${diary.staff.role === "owner" ? "owner-tabs" : ""}`}
      ...
      >
        {[
          ...
          "reports",
          ...(diary.staff.role === "owner"
            ? ["customers", "settings", "launch checks"]
            : []),
        ].map((t) => (
          <button
            key={t}
      ...
      {tab === "customers" && diary.staff.role === "owner" && <Customers />}
      ...
      {tab === "diary" && (
        <div className="diary-columns">
          {visible.map((barber) => (
            <section className="barber-diary" key={barber.id}>
              <h2>{barber.name}.</h2>
              {diary.bookings
              ...
              {!diary.bookings.some((b) => b.barber_id === barber.id) && (
                <p className="empty-diary">No trims booked.</p>
              )}
              {diary.blocks
                .filter((b) => b.barber_id === barber.id)
                .map((b) => (
