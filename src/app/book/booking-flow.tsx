...
type Details = { name: string; email: string; country: string; phone: string; marketing: boolean; preferences: string };
...
export function BookingFlow({ initialDate, initialBarber = "sean", initialService = "", catalog, policy }: { initialDate: string; initialBarber?: BarberChoice; initialService?: string; catalog: {services:Service[];hours:typeof OPENING_HOURS}; policy:{cancellation_hours:number;late_percent:number;no_show_percent:number;version:string} }) {
  ...
  async function addDates() {
    ...
    if (drafts.some(item => item.date > addDays(initialDate, 120))) return setError("Those repeats go beyond our 120-day diary. Choose fewer trims.");
    ...
    try {
      const checked = await Promise.all(drafts.map(async (item) => {
        ...
        const alternatives = makeSlots(item.date, item.barber, item.service, result.busy, catalog);
  ...
  async function finishBooking() {
    if (!formRef.current?.reportValidity() || !basket.length || !policyAccepted) return;
    ...
    try {
      ...
      setMode(result.mode);
      setDetails({ name: "", email: "", country: "GB", phone: "", marketing: false, preferences: "" });
      window.scrollTo({ top: 0, behavior: "smooth" });
    } catch (reason) { requestId.current = ""; setError(reason instanceof Error ? reason.message : "Your booking couldn’t be completed."); }
    finally { setLoading(false); }
  }
        ...
        <section className="booking-view" key={`${step}-${pending ? "pending" : "normal"}`}>
          ...
          {step === 1 && <>
            ...
            <div className="service-list">{SERVICES.map((item) => {
              const detail = (barber === "any" ? Object.values(item.barbers) : [item.barbers[barber]]).filter(Boolean);
          ...
          {step === 3 && !done && <>
            ...
            <form ref={formRef} className="details-form" onSubmit={(event: FormEvent) => { event.preventDefault(); finishBooking(); }}>
              ...
              <div className="field"><label>anything you’d like us to know? <textarea maxLength={1000} value={details.preferences} onChange={e=>setDetails({...details,preferences:e.target.value})} placeholder="Your usual cut or anything that helps your visit" /></label></div>
              <p>No deposit. Save your card securely after this step. Free cancellation until {policy.cancellation_hours} hours before; {policy.late_percent}% for late cancellation and {policy.no_show_percent}% for a no-show.</p>
              <label className="marketing"><input required type="checkbox" checked={policyAccepted} onChange={e=>setPolicyAccepted(e.target.checked)}/><span>I agree to the <a href="/cancellation-policy" target="_blank">cancellation policy</a> and authorise these fees to my saved card.</span></label>
