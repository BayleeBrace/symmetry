...
async function fetchBusy(
  date: string,
  barber: BarberChoice,
): Promise<{ busy: BusyPeriod[]; mode: "preview" | "live" }> {
  const response = await fetch(
    `/api/availability?date=${date}&barber=${barber}`,
    { cache: "no-store" },
  );
...
export function BookingFlow({
  ...
  const quickBook = SERVICES.some(
    (s) =>
      s.id === initialService &&
      (initialBarber === "any"
        ? Object.keys(s.barbers).length
        : s.barbers[initialBarber]),
  );
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        setSavedDraft(readDraft(sessionStorage.getItem(DRAFT_KEY)));
      } catch {}
  ...
  useEffect(() => {
    ...
    try {
      if (basket.length && !done)
        sessionStorage.setItem(DRAFT_KEY, encodeDraft(basket));
      else sessionStorage.removeItem(DRAFT_KEY);
    } catch {}
  }, [basket, done, draftChecked, savedDraft]);
  async function restoreDraft() {
    ...
    try {
      ...
      for (const old of savedDraft.basket) {
        ...
        if (chosen)
          restored.push({ ...chosen, date: old.date, service: old.service });
      }
      setBasket(restored);
      setSavedDraft(null);
      setPolicyAccepted(false);
      setStep(restored.length ? 3 : 0);
      setError(
        restored.length < savedDraft.basket.length
          ? "Some saved times are no longer available. We kept the ones that still work; review current prices and add replacements."
          : "Your trims are restored with current prices. Please review them and enter your details.",
      );
    } catch (e) {
      setError(
        e instanceof Error
          ? e.message
          : "Could not recover those dates. Try again.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const slots = useMemo(
    () =>
      service && availabilityReady
        ? makeSlots(date, barber, service, busy, catalog)
        : [],
    [date, barber, service, busy, catalog, availabilityReady],
  );
  const grouped = {
  ...
  const activePeriod = grouped[period].length
    ? period
  ...
  async function addDates() {
    ...
    const drafts: Draft[] = Array.from(
      { length: repeat ? count : 1 },
      (_, index) => ({ ...slot, service, date: addDays(date, repeat * index) }),
    );
    if (drafts.some((item) => item.date > addDays(initialDate, 120)))
      return setError(
        "Those repeats go beyond our 120-day diary. Choose fewer trims.",
      );
    if (basket.length + drafts.length > 12)
      return setError("You can book up to 12 trims together.");
    setLoading(true);
    setError("");
    try {
      const checked = await Promise.all(
        drafts.map(async (item) => {
          ...
          return alternatives.some((choice) => choice.time === item.time)
            ? item
            : { ...item, alternatives: alternatives.slice(0, 6) };
        }),
      );
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "Those dates couldn’t be checked.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function findNextDay(index: number) {
    ...
    try {
      for (let offset = 1; offset <= 7; offset++) {
        ...
        if (
          nextDate > addDays(initialDate, 120) ||
          !catalog.hours[parseDate(nextDate).getUTCDay()]
        )
          continue;
        const result = await fetchBusy(nextDate, pending[index].barber);
        const choice = makeSlots(
          nextDate,
          pending[index].barber,
          pending[index].service,
          result.busy,
          catalog,
        )[0];
        if (choice) {
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "The diary could not load. Try again.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function finishBooking() {
    if (!formRef.current?.reportValidity() || !basket.length || !policyAccepted)
      return;
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(result.error || "Your booking couldn’t be completed.");
      if (result.mode === "card_setup") {
        try {
          sessionStorage.setItem(DRAFT_KEY, encodeDraft(basket));
        } catch {}
  ...
  const summary =
    step === 0
      ? `with ${barber === "any" ? "any barber" : BARBERS[barber].name}`
      : step === 1 && serviceInfo
        ? serviceInfo.name
        : step === 2 && slot
          ? `${fullDate(date)} · ${clock(slot.time)} with ${BARBERS[slot.barber].name}`
          : `${basket.length} booking${basket.length === 1 ? "" : "s"} · ${money(basket.reduce((sum, item) => sum + item.price, 0))}`;
  const nextDisabled =
    loading ||
    (step === 1 &&
      (!serviceInfo || (barber !== "any" && !serviceInfo.barbers[barber]))) ||
    (step === 2 &&
      (!slot || Boolean(pending?.some((item) => item.alternatives)))) ||
    (step === 3 && !basket.length);
      ...
      <div className="booking-frame">
        {savedDraft && (
          <div className="draft-notice" role="status">
            ...
            <p>
              {savedDraft.basket.length} saved trim
              {savedDraft.basket.length === 1 ? "" : "s"}. We’ll check the dates
              and prices again. If you already saved your card, check your
        ...
        <section
          ...
          key={`${step}-${pending ? "pending" : "normal"}`}
        >
          {step === 0 && (
            <>
              <h1>your barber.</h1>
              ...
              <div className="choice-grid">
                {(
                  Object.entries(BARBERS) as [
                    BarberId,
                    (typeof BARBERS)[BarberId],
                  ][]
                ).map(([id, item]) => (
                  <button
                    key={id}
                    className={`choice ${barber === id ? "selected" : ""}`}
                    onClick={() => {
                      setBarber(id);
            ...
            <>
              ...
              <div className="service-list">
                {SERVICES.map((item) => {
                  const detail = (
                    barber === "any"
                      ? Object.values(item.barbers)
                      : [item.barbers[barber]]
                  ).filter(Boolean);
                  if (!detail.length) return null;
                  const min = Math.min(...detail.map((value) => value.price)),
                    max = Math.max(...detail.map((value) => value.price));
                  return (
                    <button
                    ...
                    >
                      <span>
                        {item.name}
                        <small>
                          {Math.min(...detail.map((value) => value.duration))}
                          {detail.some(
                            (value) => value.duration !== detail[0].duration,
                          )
                            ? `–${Math.max(...detail.map((value) => value.duration))}`
                            : ""}{" "}
                          min
                        </small>
                      </span>
            ...
            <>
              ...
              <p className="intro">
                {pending
                  ...
                  : `${serviceInfo.name} · ${barber === "any" ? "your barber is shown with each time" : BARBERS[barber].name}`}
              </p>
              {pending ? (
                pending.map((item, index) => (
                  <article className="resolution" key={`${item.date}-${index}`}>
                    <h3>
                      {fullDate(item.date)} ·{" "}
                      {item.alternatives
                        ? "choose another time"
                ...
                <>
                  <div className="month-nav">
                    <button
                      ...
                      disabled={month <= initialDate.slice(0, 7)}
                    ...
                    <button
                      onClick={() => setMonth(shiftMonth(month, 1))}
                      disabled={month >= addDays(initialDate, 120).slice(0, 7)}
                  ...
                  <div className="calendar">
                    ...
                    {days.dates.map((value) => {
                      const enabled =
                        value >= initialDate &&
                        value <= addDays(initialDate, 120) &&
                        Boolean(catalog.hours[parseDate(value).getUTCDay()]);
                      return (
                        <button
                          className={`date-cell ${date === value ? "selected" : ""}`}
                          disabled={!enabled}
                          onClick={() => chooseDate(value)}
                          aria-label={fullDate(value)}
                  ...
                  <h2 ref={timesRef} className="times-heading">
                    {fullDate(date)}
                  </h2>
                  <div className="periods">
                    {(["morning", "afternoon", "evening"] as const).map(
                      (name) => (
                        <button
                          className={`period ${activePeriod === name ? "selected" : ""}`}
                          disabled={!grouped[name].length}
                  ...
                  {!loading && !slots.length && (
                    <p className="error-message">
                      No space on this day. Try another date or{" "}
          ...
          {step === 3 && !done && (
            <>
              ...
              {basket.map((item, index) => (
                ...
                >
                  <h3>
                    {fullDate(item.date)} · {clock(item.time)}
                  </h3>
                  <p>
                    {
                      SERVICES.find(
                        (serviceItem) => serviceItem.id === item.service,
                      )?.name
                    }{" "}
                    with {BARBERS[item.barber].name} · {money(item.price)}
                  </p>
                  <button
                    onClick={() =>
                      setBasket((items) =>
              ...
              >
                <div className="field">
                  <label>
                    ...
                    <input
                      required
                      minLength={2}
                      autoComplete={mode === "live" ? "name" : "off"}
                      value={details.name}
                      onChange={(event) =>
                ...
                <div className="field">
                  <label>
                    ...
                    <input
                      required
                      type="email"
                      autoComplete={mode === "live" ? "email" : "off"}
                      value={details.email}
                      onChange={(event) =>
                  ...
                  <label>
                    ...
                    <span className="phone-row">
                      ...
                      <input
                        ...
                        type="tel"
                        inputMode="tel"
                        autoComplete={mode === "live" ? "tel-national" : "off"}
                ...
                <div className="field">
                  <label>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setDetails({ ...details, preferences: e.target.value })
                      }
                      placeholder="Your usual cut or anything that helps your visit"
                    />
                  </label>
                </div>
                <p>
                  No deposit. Save your card securely after this step. Free
                  cancellation until {policy.cancellation_hours} hours before;{" "}
                  {policy.late_percent}% for late cancellation and{" "}
                  {policy.no_show_percent}% for a no-show.
                </p>
                <label className="marketing">
                  ...
                  <span>
                    ...
                    and authorise these fees to my saved card.
                  </span>
                </label>
                <p className="privacy-note">
                  We use your details to manage your booking. Read our{" "}
                  <a href="/privacy" target="_blank" rel="noreferrer">
                    privacy notice
                  </a>
                  . Stripe securely stores your card details.
                </p>
                <label className="marketing">
                  <input
                    type="checkbox"
          ...
          {step === 3 && done && (
            <>
              ...
              <div className="success-card">
                <p>
                  {mode === "live"
                    ? "Your trims are reserved. Keep your management link below to view or change them."
