-- Clients and checkout. Safe to run whether or not 20260908000000_rebook_reminders
-- or the other copy's 20260910072152_customer_directory ran first: every step skips
-- what already exists. Groups identical guest records into one client, reuses a client
-- on later bookings (exact name, email and mobile), keeps the reminder consent wording,
-- and records how a trim was paid for at checkout.
...
alter table public.customers add column if not exists last_nudged_at timestamptz;
alter table public.customers add column if not exists directory_parent_id uuid references public.customers(id);
with matches as (
 select id, first_value(id) over (
  partition by name,email,phone,phone_country,preferences order by created_at,id
 ) canonical_id from public.customers c
 where auth_user_id is null and directory_parent_id is null and email <> '' and phone <> ''
...
alter table public.bookings add column if not exists paid_by text check (paid_by in ('card','cash','other'));
